...
function InterpreteEdit() {
    ...
    const recuperarInterprete = () => {
        fetch(`http://localhost:3333/interprete/readById/${idInterprete}`, {
            method: 'GET',
        }).then((res) => {
            if (res.ok) {
                return res.json();
    ...
    async function salvarDados(dto) {
        await fetch('http://localhost:3333/interprete/edit', {
            method: 'PUT',
            headers: {
        ...
        <div className="Signin">
            ...
            <div className='LoginContainer' style={{maxWidth: '600px', margin: '5em auto'}}>
                <form>
                    ...
                    <button onClick={updateInterprete} style={{backgroundColor: "#0d6efd", border: "none", borderRadius: "5px", color: "white", width: "100px", height: "38px"}}>Editar</button>
            </div>
            <ToastContainer/>
        </div>
    )
